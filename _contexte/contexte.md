...
`conversation_test.py` accepte désormais `--duration <minutes>` : au-delà du délai, plus aucun nouveau tour n'est envoyé mais le tour en cours va à son terme ; `--turns` devient illimité par défaut si `--duration` est utilisé sans lui. Commande Claude Code `/stop_opencode` créée pour arrêter proprement une session interrompue de force (reconstruction du `manifest.json`, nettoyage des flags de contrôle) — non encore testée en conditions réelles.
En attente : fiabilité OCR sur d'autres valeurs de contexte que 7 %/50 % ; `/stop_opencode` et le mode `--duration` à tester jusqu'au bout ; `ROADMAP_PATH` de `conversation_test.py` pointe vers une roadmap jetable de test, pas une roadmap fonctionnelle.
