...
Pont OpenCode fiabilisé : overlay de sélection de zone corrigé (multi-écrans, y compris l'écran de l'UI), vérification image retirée sur les clics fragiles de la macro `OPENCODE-envoyer`, blocage automatique + `/compact` + reprise quand le contexte OCR atteint le seuil (`compact_opencode`). Trois sessions `conversation_test.py` consécutives réussies (9 tours au total, 0 échec macro).
Bannière de contrôle rouge clignotante ajoutée dans l'UI (flags fichiers `mark_control_active`/`mark_session_active`) ; correction pour rester affichée tout le workflow non encore retestée en conditions réelles (`tests_manuels.md`).
...
En attente : fiabilité OCR sur d'autres valeurs de contexte que 7 %/50 %, retest de la bannière (`tests_manuels.md`).
